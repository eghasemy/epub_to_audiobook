...
def get_supported_tts_providers() -> List[str]:
    return [TTS_AZURE, TTS_OPENAI, TTS_EDGE, TTS_PIPER, TTS_XTTS]
...
def get_tts_provider(config) -> BaseTTSProvider:
    if config.tts == TTS_AZURE:
        ...
        from audiobook_generator.tts_providers.xtts_tts_provider import XTTSTTSProvider

        return XTTSTTSProvider(config)
    else:
